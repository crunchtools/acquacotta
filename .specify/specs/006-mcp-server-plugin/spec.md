...
- **FR-003a**: Per-user revocation MUST be implemented via a per-user token epoch (a revocation watermark timestamp) stored alongside the user's existing `location` record. Each token carries its `issued_at`; the server MUST reject any token whose `issued_at` predates the user's current epoch. Disable and Regenerate advance the epoch. This watermark is access metadata, not user content, and is the only per-user state the server keeps.
...
- **FR-008**: Todos tools MUST include: `list_todos`, `list_todo_lists`, `create_todo`, `complete_todo`, `update_todo`.
- **FR-009**: Pomodoro tools MUST include: `get_pomodoros`, `get_time_summary`, and `tag_pomodoro_to_ticket`.
